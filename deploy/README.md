...
## Ошибка «Profile columns not found» (профиль на проде)

Если при сохранении профиля (Пол, Дата рождения и т.д.) на сайте появляется ошибка **«Profile columns not found. Run scripts/migrate_users_profile.sql»** — на продовой БД не применена миграция профиля. Выполните **один раз на VPS** (подставьте своего пользователя и имя БД из `.env`):
...
Если БД на другом хосте или другой пользователь — укажите их в `psql` (`-h`, `-U`, `-d`). После применения миграции перезапуск API не обязателен — изменения схемы подхватываются при следующем запросе.
